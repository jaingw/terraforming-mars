...
export abstract class Colony implements IColony {
    ...
    public trade(player: Player, tradeOptions: TradeOptions = {}, bonusTradeOffset = 0): void {
      ...
      for (const somePlayer of player.game.getPlayers()) {
        if (somePlayer.cardIsInEffect(CardName.PLANT_SMUGGLING)) {
          somePlayer.addResource(Resource.PLANTS, 1);
        }
      }
    ...
    // 殖民收入 贸易收入
    public giveBonus(player: Player, bonusType: ColonyBenefit, quantity: number, resource: Resource | undefined, isGiveColonyBonus: boolean = false): undefined | PlayerInput {
      ...
      case ColonyBenefit.LOSE_RESOURCES:
        if (resource === undefined) throw new Error('Resource cannot be undefined');
        player.deductResource(resource, quantity, {unforce: true});
        break;
