...
export class TempestConsultancy extends Card implements ICorporationCard {
  ...
  public canAct(player: Player) {
    return player.tags.count(Tag.MOON) >= 5 && TurmoilUtil.getTurmoil(player.game).getAvailableDelegateCount(player.id) > 0;
  }

  public action(player: Player) {
    let count = Math.floor(player.tags.count(Tag.MOON) / 5);
    count = Math.min(count, 3);
    count = Math.min(count, TurmoilUtil.getTurmoil(player.game).getAvailableDelegateCount(player.id));
    if (count > 0) {
