...
import {Space} from './BoardBuilder';
...
export abstract class Board {
  ...
  public getAvailableSpacesForGreenery(player: Player): Array<ISpace> {
    ...
    const spacesForGreenery = spacesOnLand
      .filter((space) => this.getAdjacentSpaces(space).find((adj) => {
        return adj.tile !== undefined &&
        adj.player === player &&
        adj.tile.tileType !== TileType.OCEAN &&
        !AresHandler.hasHazardTile(adj) !== undefined;
      }));
  ...
  public static deserializeSpace(space: SerializedSpace, players: Array<Player>): ISpace {
    ...
    if (space.player) {
      player = players.find((player) => player.id === space.player?.id);
  ...
  public static deserializeSpaces(spaces: Array<SerializedSpace>, players: Array<Player>): Array<ISpace> {
    const spacestmp = spaces.map((space) => Board.deserializeSpace(space, players));
    if (spacestmp.find((space) => space.id === SpaceName.TRANTOR) === undefined) {
      spacestmp.push(Space.colony(SpaceName.TRANTOR));
    }
    return spacestmp;
  }
}
