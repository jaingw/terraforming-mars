...
export class SendDelegateToArea implements DeferredAction {
  constructor(
        ...
        public nbr: number = 1,
        public replace: NeutralPlayer | Player | undefined = undefined,
    ...
    sendDelegate.options = parties.map((party) => new SelectOption(
      ...
      () => {
        ...
        if (this.nbr > 1 && this.fromLobby) { // For card: Cultural Metropolis
                    this.game.turmoil?.sendDelegateToParty(this.player, party.name, this.game, true);
                    for (let i = 0; i < this.nbr - 1; i++) {
                        this.game.turmoil?.sendDelegateToParty(this.player, party.name, this.game, false);
                    }
        } else {
          for (let i = 0; i < this.nbr; i++) {
            if (this.replace) {
                        ...
                        this.game.turmoil?.sendDelegateToParty(this.player, party.name, this.game, this.fromLobby);
          }
        }
