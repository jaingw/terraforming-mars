...
export class MonsInsurance extends Card implements ICorporationCard {
  ...
  public static payDebt(player: Player, claimant : Player | undefined) {
    if (player !== claimant) {
  ...
  public static resolveInsurance(claimant: Player) {
    ...
    if (game.monsInsuranceOwner !== undefined && game.monsInsuranceOwner !== claimant) {
  ...
  public static resolveInsuranceInSoloGame(potentialInsurer: Player) {
    if (potentialInsurer.game.monsInsuranceOwner === potentialInsurer) {
