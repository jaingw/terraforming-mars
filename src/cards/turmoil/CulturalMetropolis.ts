...
export class CulturalMetropolis extends Card implements IProjectCard {
  ...
  public canPlay(player: Player): boolean {
    if (player.game.turmoil !== undefined) {
      ...
      return player.game.turmoil.canPlay(player, PartyName.UNITY) &&
              player.getProduction(Resources.ENERGY) >= 1 &&
              (player.game.turmoil.getDelegates(player) > 1 ||
                (player.game.turmoil.getDelegates(player) === 1 && player.game.turmoil.lobby.has(player.id)));
    }
    return false;
  }
  ...
  public play(player: Player) {
    ...
    if (player.game.turmoil!.getDelegates(player) > 1) {
      player.game.defer(new SendDelegateToArea(player, title, {count: 2, source: 'reserve'}));
    } else if (player.game.turmoil!.getDelegates(player) === 1 && player.game.turmoil!.lobby.has(player.id)) {
