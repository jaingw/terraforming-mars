...
export class VoteOfNoConfidence extends Card implements IProjectCard {
  ...
  public canPlay(player: Player): boolean {
    if (player.game.turmoil !== undefined) {
      if (player.game.turmoil!.getDelegatesInReserve(player) === 0) return false;

      const parties = player.game.turmoil.parties.filter((party) => party.partyLeader === player);
  ...
  public play(player: Player) {
    if (player.game.turmoil !== undefined) {
            ...
            const index = player.game.turmoil.delegateReserve.indexOf(player);
            if (index > -1) {
