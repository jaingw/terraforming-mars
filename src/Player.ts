...
export class Player implements ISerializable<SerializedPlayer> {
  ...
  private resolveMonsInsurance() {
    if (this.game.monsInsuranceOwner !== undefined && this.game.monsInsuranceOwner !== this) {
      ...
      this.game.monsInsuranceOwner.setResource(Resources.MEGACREDITS, -3);
      if (retribution > 0) {
  ...
  public runDraftPhase(initialDraft: boolean, playerName: string, passedCards?: Array<IProjectCard>): void {
    ...
    if (passedCards === undefined) {
      if (!initialDraft) {
        let cardsToDraw = 4;
        if (LunaProjectOffice.consume(this) || this.isCorporation(CardName._TERRALABS_RESEARCH_)) {
          cardsToDraw = 5;
          cardsToKeep = 2;
        }
  ...
  public runResearchPhase(draftVariant: boolean): void {
    ...
    if (!draftVariant) {
      ...
      else this.dealCards(LunaProjectOffice.consume(this) ? 5 : 4, dealtCards);
    } else {
  ...
  public getActions() {
    ...
    if (this.game.gameOptions.turmoilExtension && this.game.turmoil !== undefined) {
      ...
      } else if (this.isCorporation(CardName.INCITE) && this.canAfford(3) && this.game.turmoil.getDelegates(this) > 0) {
        sendDelegate = new SendDelegateToArea(this, 'Send a delegate in an area (3 MC)', {cost: 3});
      } else if (this.canAfford(5) && this.game.turmoil!.getDelegates(this) > 0) {
  ...
  public static deserialize(d: SerializedPlayer): Player {
    ...
    if (d.corporationCard !== undefined) {
      ...
      if (player.corporationCard instanceof _MorningStarInc_) {
        if ((d.corporationCard as any).isUsed === true) player.corporationCard.isUsed = true;
      }
    } else {
