...
export class GameLoader {
    ...
    public addGame(game: Game): void {
        ...
        for (const player of game.getAllPlayers()) {
            ...
            if (user !== undefined) {
                let gameids = this.usersToGames.get(user.id);
                if (gameids !== undefined) {
                    gameids.push(game.id);
                } else {
                    gameids = [];
                    gameids.push(game.id);
                    this.usersToGames.set(user.id, gameids);
                }
            }
        }
    }
    ...
    public getGameByPlayerId(playerId: string, cb: (game: Game | undefined) => void): void {
        if (this.loadedGames === true || this.playerToGame.has(playerId)) {
            ...
            if(game !== undefined && this.games.get(game.id) !== undefined){
                cb(game);
            }else{
                cb(undefined);
            }
                ...
                Database.getInstance().restoreGameLastSave(
                    ...
                    (err) => {
                        ...
                        if (err) {
                            ...
                            for (const player of gameToRebuild.getAllPlayers()) {
