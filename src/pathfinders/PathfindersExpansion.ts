...
export class PathfindersExpansion {
  ...
  // from 和 name 二选一
  private static raiseTrackEssense(tag: PlanetaryTag, from: Player | undefined, name: GlobalEventName | undefined, game: Game, steps: number = 1, gainRewards: boolean = true): void {
    ...
    if (from !== undefined) {
      ...
      game.log('Global Event ${0} raised the ${1} planetary track ${2} ${3}', (b) => {
        b.globalEventName(name).string(tag).number(distance).string(distance > 1 ? 'steps' : 'step');
      });
    }
