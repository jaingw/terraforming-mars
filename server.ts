    ...
    if (req.url !== undefined) {
        if (req.method === "GET") {
            ...
            gameLoader.getGameByPlayerId(playerId, (game) => {
                ...
                const user = gameLoader.userNameMap.get(player.name);
...
export function generateRandomGameId(): string {
...
function apiGetGames(
    ...
    if (! gameLoader.loadedGames ) {
        console.warn("loading");
        route.notFound(req, res);
        return ;
    }
    const answer: Array<any> = [];
    const games = gameLoader.games;
...
function createGame(req: http.IncomingMessage, res: http.ServerResponse): void {
    ...
    req.once("end", function () {
        try {
            ...
            res.setHeader("Content-Type", "application/json");
            res.write(getGame(game,gameReq.userId));
            res.end();
        } catch (error) {
            route.internalServerError(req, res, error);
        }
...
export function getPlayer(player: Player, game: Game, block: boolean): string {
  ...
  try{
    ...
    const output = {
        ...
        passedPlayers: game.getPassedPlayers(), // JSON stringify does not honor sets
...
function getWaitingFor(
    ...
    switch (waitingFor.inputType) {
        ...
        case PlayerInputTypes.SELECT_CARD:
            ...
            result.cards = getCardsAsCardModel(selectCard.cards, selectCard.showResouces);
            result.maxCardsToSelect = selectCard.maxCardsToSelect;
            result.minCardsToSelect = selectCard.minCardsToSelect;
            break;
...
function getGame(game: Game, userId : string = ""): string {
