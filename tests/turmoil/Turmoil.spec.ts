...
describe('Turmoil', function() {
  ...
  it('Correctly send delegate from the reserve', function() {
    ...
    turmoil.sendDelegateToParty(player, PartyName.GREENS, game, 'reserve');
  ...
  it('serializes and deserializes keeping players', function() {
    ...
    turmoil.parties[0].delegates = ['NEUTRAL', 'NEUTRAL', game.getAllPlayers()[0]];
    // const serialized = JSON.parse(JSON.stringify(turmoil.serialize()));
    // const deserialized = Turmoil.deserialize(serialized, game);
    // expect(deserialized.parties[0].getPresentPlayers()).to.have.members(['NEUTRAL', game.getAllPlayers()[0]]);
  });
  ...
  it('deserialization', () => {
    const json = {
      ...
      'dominantParty': 'Unity',
      'lobby': [{'id': 'blue-id'}],
      'delegateReserve': ['NEUTRAL', 'NEUTRAL'],
      'parties': [
    ...
    expect(s.chairman).eq('NEUTRAL');
    // const t = Turmoil.deserialize(s, game);
    ...
    // expect(t.delegateReserve).deep.eq(['NEUTRAL', 'NEUTRAL']);
